 ...
 */
/**
 * categories - articulo relation edit block
 *
 * @category    MyExtension
 * @package     MyExtension_Mymodule
 * @author      Ultimate Module Creator
 */
class MyExtension_Mymodule_Block_Adminhtml_Catalog_Category_Tab_Articulo extends MyExtension_Mymodule_Block_Adminhtml_Articulo_Tree
{
    ...
    {
        parent::__construct();
        $this->setTemplate('myextension_mymodule/catalog/category/tab/articulos.phtml');
    }
    ...
    /**
     * Return array with articulos IDs which the category is linked to
     *
     * @access public
     * @return array
     * @author Ultimate Module Creator
     */
    public function getArticulosIds()
    {
        if (is_null($this->_articulosIds)) {
            $articulos = Mage::helper('myextension_mymodule/category')
                ->getSelectedArticulos($this->getCategory());
            $ids = array();
    ...
    {
        return implode(',', $this->getArticulosIds());
    }
    ...
    {
        $root = $this->getRoot();
        if ($root && in_array($root->getId(), $this->getArticulosIds())) {
    ...
    protected function _getNodeJson($node, $level = 1)
    {
        ...
        if (in_array($node->getId(), $this->getArticulosIds())) {
            $item['checked'] = true;
        }
        return $item;
    }
    ...
    /**
     * Returns array with nodes those are selected (contain current articulo)
     *
     * @access protected
     * @return array
     * @author Ultimate Module Creator
    ...
    {
        return $this->getUrl(
            '*/mymodule_articulo_catalog_category/articulosJson',
            array('_current' => true)
        );
    }

    /**
     * Return distinct path ids of selected articulo
     *
     * @access public
    ...
    public function buildNodeName($node)
    {
        ...
        $result .= '<a target="_blank" href="'.
            $this->getUrl('adminhtml/mymodule_articulo/index', array('id'=>$node->getId(), 'clear'=>1)).
